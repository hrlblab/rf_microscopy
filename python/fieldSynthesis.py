...
if __name__ == "__main__":
    ...
    for c in range(0, n):
        if selected_columns[c]:
            latticeFourierMask[:, c] = True

    fieldSynthesisProfile = ft.fftshift(ft.ifft(ft.ifftshift(latticeFourierMask)))
    fieldSynthesisProfile = [sum(i) for i in fieldSynthesisProfile]
    ...
    if period == round(period):
        latticeDithered = np.zeros(len(lattice));
        for s in range(math.floor(-period / 2), math.floor(period / 2) - 1):
            latticeDithered = latticeDithered + np.roll(lattice, s)
    ...
    ax[0, 0].imshow(abs(latticeFourierMask), cmap='plasma')
    ax[0, 0].set_title('Electric field in pupil')

    ax[0, 0].imshow(abs(latticeDithered_hat), cmap='plasma')
    ax[0, 1].set_title('Fourier components of lattice intensity')

    ax[0, 2].imshow(lattice_hat, cmap='plasma')
    ax[0, 2].set_title('Fourier components of dithered lattice intensity')

    ax[1, 0].imshow(lattice_efield, cmap='plasma')
    ax[1, 0].set_title('Electric field of lattice at focal plane')
    plt.xlim([-75, 75] + len(dispRange) / 2 + 1)
    plt.ylim([-75, 75] + len(dispRange) / 2 + 1)

    ax[1, 2].imshow(lattice, cmap='plasma')
    ax[1, 1].set_title('Intensity of lattice')
